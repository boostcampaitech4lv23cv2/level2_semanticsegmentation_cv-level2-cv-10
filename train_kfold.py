...
def train(args):
    ...
    for fold in range(folds):
        # -- data_set
        train_path = os.path.join(dataset_path, f'kfold/train_fold{fold}.json')
        val_path = os.path.join(dataset_path, f'kfold/val_fold{fold}.json')
        ...
        for epoch in range(args.epochs):
            ...
            if (epoch + 1) % val_every == 0:
                ...
                if val_mIoU > best_mIoU:
                    ...
                    best_mIoU = val_mIoU
                    save_model(model, saved_dir, file_name=f'best_{fold}.pth')
                    counter = 0
                else:
                    counter += 1
