...
def split_dataset():
    ...
    for idx, (train_index, val_index) in tqdm(enumerate(cv.split(X, y, groups)), total=5):
        ...
        output_train_json = os.path.join(output_dir, f"train_fold{idx}.json")
        output_val_json = os.path.join(output_dir, f"val_fold{idx}.json")

        with open(output_train_json, "w") as train_writer:
            json.dump(train_dict, train_writer)

        with open(output_val_json, "w") as val_writer:
            json.dump(val_dict, val_writer)
